...
namespace CBRE.Common.Mediator
{
    ...
    public static class Mediator
    {
        ...
        public static bool ExecuteDefault(object obj, string message, object parameter)
        {
            ...
            if (method != null)
            {
                ISynchronizeInvoke sync = obj as ISynchronizeInvoke;
        ...
        {
            if (!Listeners.ContainsKey(message)) return;
            List<WeakReference> list = new List<WeakReference>(Listeners[message]);
            foreach (WeakReference reference in list)
            {
                ...
                {
                    MethodInfo method = reference.Target.GetType().GetMethod("Notify", new[] { typeof(string), typeof(object) });
                    if (method != null)
                    {
                        try
                        {
